...
{
public:
    SGLPCD8544(PinName DC, PinName CE, PinName RST, PinName SPI_MOSI, PinName SPI_MISO, PinName SPI_SCK);
    void backlight(bool onoff);
    void init();
    void reset();
    void send_data(uint8_t data);
    void send_command(uint8_t cmd);
    void drawPixel(uint16_t x, uint16_t y, uint16_t color = BLACK, Mode mode = Mode::px_copy) override;
